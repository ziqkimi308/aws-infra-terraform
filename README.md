...
**Issue:** 
Website returned `ERR_CONNECTION_TIMED_OUT` — Apache never started.

**Investigation:** 
Security group was correctly attached (`tf-devops-ec2-sg`, not default) with port 80 open, ruling out network config. SSH into the instance showed `apache2.service could not be found`.

**Root cause:** 
`sudo cloud-init status` showed `status: error`, and `/var/log/cloud-init-output.log` showed the `cc_scripts_user` module failed. Mixed tabs/spaces inside the Terraform `<<-EOF` heredoc corrupted the shebang line, so the user_data script never executed.

**Fix:** 
Removed all leading whitespace from heredoc lines in `user_data`. Recreated the instance (`terraform destroy` → `terraform apply`) to pick up the corrected script. Verified with `curl` and browser — Apache served the page correctly on the new IP.
